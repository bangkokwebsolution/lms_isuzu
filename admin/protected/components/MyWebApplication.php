...
class MyWebApplication extends CWebApplication {
    ...
    public function getUploadUrl($module) {
    	...
    	$path = "";
    	$baseUrl = $this->getBaseUrl(true)."/../".$this->folderUpload."/";
    	$path = $baseUrl.$module."/";
        return $path;
    }
    ...
    // วิธีเรียกใช้ Yii::app()->checkShowImage("edify",$resultEdify->cms_picture)
    // วิธีเรียกใช้ Yii::app()->checkShowImage(null,$resultEdify->cms_picture) ถ้าดึงรูปจาก Folder ที่ชื่อเดียวกับ Controller
    ...
    // Upload Image
    public function uploadImageNutt($module,$model,$nameHtml,$oldImage = "") {
        if($module == null)
            $module = $this->controller->id;
        ...
        if($uploadedFile){
            ...
            $thumb->adaptiveResize($this->params['IMAGE_SIZE']['FULL']['WIDTH'],$this->params['IMAGE_SIZE']['FULL']['HEIGHT']);
