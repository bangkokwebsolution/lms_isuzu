 ...
 * @since 1.0
 */
class ZTreeViewLesson extends CTreeView
{
	/**
	...
	public static function saveDataAsHtml($data)
	{
		...
		if(is_array($data))
		{
			foreach($data as $node)
			{
				...
				if(isset($node['data']))
					$options['data-id']=$node['data'];

                if(isset($node['lesson']))
                    $options['data-fileid']=$node['lesson'];

				$html.=CHtml::tag('li',$options,false,false);
				$html.="<div class='dd-handle'>";
				$html.="<div class=\"well well-sm\">";
				$html.="<div class=\"row\">";
				// $html.="<div class=\"col-xs-3 col-md-3 text-center\">";
				// $html.="<img src=\"http://bootsnipp.com/apple-touch-icon-114x114-precomposed.png\" alt=\"bootsnipp\" class=\"img-rounded img-responsive\"/>";
    //             $html.="</div>";
                $html.="<div class=\"col-xs-12 col-md-12 section-box\">";
                // $html.="<h5>".$node['code'].' | '.$node['text']."</h5>";
                $html.="<h5>".$node['text']."</h5>";
                // $html.="<p>".$node['text']."</p>";
				$html.="</div>";
				$html.="</div>";
				$html.="</div>";
				...
				$html.='<a href="'.Yii::app()->createUrl('lesson/update',array('id'=>$node['lesson'])).'" class="btn btn-warning btn-edt"><i class="fa fa-pencil" aria-hidden="true"></i></a>
';
				// $html.="<a href='".Yii::app()->createUrl('lesson/update',array('id'=>$node['data']))."' class='btn btn-warning btn-edt'><i class='fa fa-pencil' aria-hidden='true'></i></a>";
