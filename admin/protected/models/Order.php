...
class Order extends AActiveRecord
{
	public $user_name;
	...
	{
		$check = false;
		if(Controller::PButton(array("Order.*", "Order.Delete")) == true)
		{
			if($this->con_admin == '0')
			{
				$check = true;
	...
	public function getDateConfirm()
	{
		if(isset($this->order_date_add))
			$date = ClassFunction::dateThaiShort($this->order_date_add);
		else
    ...
    public function checkScopes($check = 'scopes')
    {
    	if ($check == 'scopes')
    	{
		    $checkScopes =  array(
	    		'alias'=>'order',
	    		'order' => 'order.update_date desc',
	    		'condition' => ' order.active = "y" AND order.con_user = "1" ',
		    );	
    	}
    	else
    	{
	...
	public function scopes()
    {
		...
		$Access = Controller::SetAccess( array("Order.*") );
		if($Access == true)
		{
			$scopes =  array( 
				'ordercheck' => $this->checkScopes('scopes') 
			);
		}
		else
		{
	...
	public function attributeLabels()
	{
		return array(
			...
			'order_countall' => 'จำนวนของทั้งหมด',
			'order_file' => 'รูปไฟล์เอกสาร',
			'order_point' => 'ได้แต้ม',
			'order_ems' => 'EMS ไปรษณีย์',
			'order_date_add'=>'วันที่แจ้งการโอน',
			'order_date_time' =>'เวลาการโอนเงิน',
			'create_date' => 'วันที่สั่งซื้อ',
			'order_bank' => 'ธนาคารที่โอน',
			'create_by' => 'Create By',
			'update_date' => 'วันที่แจ้งการโอน',
			'update_by' => 'Update By',
			'con_user' => 'ยืนยันการโอนเงิน',
			'con_admin' => 'ยืนยันการโอน',
			'active' => 'Active',
		);
	}
