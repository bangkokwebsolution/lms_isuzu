...
class Orderonline extends AActiveRecord
{
	public $user_name;
	...
	{
        return array(
        	'orderonlines'=>array(self::HAS_ONE, 'OrderDetailonline', 'order_id'),
	...
	{
		$check = false;
		if(Controller::PButton(array("Orderonline.*", "Orderonline.Delete")) == true)
		{
			if($this->con_admin == '0')
			{
				$check = true;
    ...
    public function checkScopes($check = 'scopes')
    {
    	if ($check == 'scopes')
    	{
		    $checkScopes =  array(
		    	'alias'=>'orderonline',
		    	'order' => 'orderonline.update_date desc',
		    	'condition' => ' orderonline.active = "y" ',
		    );	
    	}
    	else
    	{
	...
	public function scopes()
    {
		...
		$Access = Controller::SetAccess( array("Orderonline.*") );
		if($Access == true)
		{
			$scopes =  array( 
				'orderonlinecheck' => $this->checkScopes('scopes') 
			);
		}
		else
		{
