...
class LearnNote extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('course_id, gen_id, user_id, lesson_id, file_id, created_by, updated_by', 'numerical', 'integerOnly'=>true),
			array('note_time, note_times', 'length', 'max'=>255),
			array('active', 'length', 'max'=>1),
			array('note_text, created_date, update_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('note_id, course_id, gen_id, user_id, lesson_id, file_id, note_time, note_text, note_times, active, created_by, created_date, updated_by, update_date', 'safe', 'on'=>'search'),
		);
	}
