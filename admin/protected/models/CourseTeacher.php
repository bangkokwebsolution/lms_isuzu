...
class CourseTeacher extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('survey_header_id,title','required'),
			array('teacher_id, course_id, survey_header_id', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, teacher_id, course_id, survey_header_id, title,schedule_id,group_user', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'title' => 'หัวข้อ',
			'schedule_id' => 'รหัสตารางเรียน',
			'group_user' => 'กลุ่มผู้ใช้งาน'
		);
	}
