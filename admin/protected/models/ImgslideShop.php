...
class ImgslideShop extends AActiveRecord
{
    ...
    public function checkScopes($check = 'scopes')
    {
    	if ($check == 'scopes')
    	{
		    $checkScopes =  array(
			    'alias' => 'imgslideshops',
			    'order' => ' imgslideshops.imgslide_id DESC ',
			    'condition' => ' imgslideshops.active ="y" ',
		    );	
    	}
    	else
    	{
	...
	public function scopes()
    {
		...
		$Access = Controller::SetAccess( array("ImgslideShop.*") );
		if($Access == true)
		{
			$scopes =  array( 
				'imgslideshopcheck' => $this->checkScopes('scopes') 
			);
		}
		else
		{
			...
			{
			    $scopes = array(
		            'imgslideshopcheck'=>array(
			    		'alias' => 'imgslideshops',
			    		'condition' => ' imgslideshops.create_by = "'.Yii::app()->user->id.'" AND imgslideshops.active ="y" ',
			    		'order' => ' imgslideshops.imgslide_id DESC ',
		            ),
			    );
			}
