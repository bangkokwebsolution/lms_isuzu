...
//			exit();
    ...
    {
        $model=new Coursequestion('import');

        if(isset($_POST['Coursequestion']))
        {
            $phpExcelPath = Yii::getPathOfAlias('ext.phpexcel.Classes');
            include($phpExcelPath . DIRECTORY_SEPARATOR . 'PHPExcel.php');

            $model->attributes=$_POST['Coursequestion'];
...
//				echo '</pre><hr />';
                ...
                $this->redirect(array('Coursegrouptesting/index'));
	...
	//***** Save Coursequestion All *****//
	private function SaveQuesAll($Qtype,$Ctype,$IdGroup)
	{
		$modelQues = new Coursequestion;
		$modelChoice = new Coursechoice;
		...
		$modelQues->attributes = $_POST['Coursequestion'];
		...
		for($i=0; $i<=(int)$count; $i++)
		{
			...
			if(isset($modelQues->ques_title[$i]) && $modelQues->ques_title[$i] != '')
			{
				...
				for($z=0; $z<(int)$countChoice; $z++)
				{
					$modelChoice = new Coursechoice;
					$sum = $z+1;
					if(isset($_POST['Coursechoice']['choice_answer'][$i][$z]))
					{
						if($_POST['Coursechoice']['choice_answer'][$i][$z] != 0)
						{
							if(isset($_POST['Coursechoice']['choice_detail'][$i][$z]))
							{
								$modelChoice->ques_id = $SaveQues->ques_id;
								$modelChoice->choice_answer = $_POST['Coursechoice']['choice_answer'][$i][$z];
								$modelChoice->choice_detail = CHtml::encode($_POST['Coursechoice']['choice_detail'][$i][$z]);
							}
						}
						else
						{
					...
					if(isset($_POST['Coursechoice']['choice_detail'][$i][$z]))
					{
						...
						$modelChoice->choice_type = $_POST['Coursechoice']['choice_type'][$i][$z];
						$modelChoice->save();
					}
				}
			}
	...
	public function actionShowPoint()
	{
		if(isset($_POST['Count']))
		{
			...
			for($i=0; $i<(int)$count; $i++)
			{
			    ...
			    $Coursequestion = Coursequestion::model()->find($criteria);

				$scoreGood[$i] = 0; //Good
				$scoreWrong[$i] = 0; //Wrong
				$scoreChoice[$i] = 0;
				$countChoice = Coursechoice::model()->findAll("ques_id=:ques_id", array(
					"ques_id" =>$Coursequestion->ques_id,
				));
			 	foreach($countChoice as $z=>$resultChoice)
			 	{
				    if($resultChoice->choice_answer == 1){ $scoreChoice[$i] = $scoreChoice[$i]+1; }
			    	if($_POST['Coursechoice']['choice_answer'][$i][$z] == $resultChoice->choice_answer){
			    		$scoreGood[$i] = $scoreGood[$i]+1;
			    		// echo '<font color=gree>Y Get '.$_POST['Coursechoice']['choice_answer'][$i][$z].'  DB '
			    		//  .$resultChoice->choice_answer.'</font><br>';
			    	}else{
			    		if($_POST['Coursechoice']['choice_answer'][$i][$z] == 1){
				    		$scoreWrong[$i] = $scoreWrong[$i]+1;
					    		// echo 'N Get '.$_POST['Coursechoice']['choice_answer'][$i][$z].'  DB '
					...
					if(($questionType != 'textarea') && ($questionType != 'dropdown')) {
						...
						if(count($update_ids) > 0){
							foreach ($update_ids as $choice_id) {
								$choiceModel = Coursechoice::model()->findByPk($choice_id);
//								$choiceModel->ques_id = $questionModel->ques_id;
								$choiceModel->choice_detail = $choiceTitle[$choice_id];
//								$choiceModel->choice_type = $questionType;
								if (in_array($choice_id, $answerValidCheckArray)) {
						...
						if(count($delete_ids) > 0){
							foreach ($delete_ids as $choice_id) {
								if ($choiceModel = Coursechoice::model()->findByPk($choice_id)) {
									$choiceModel->delete();
								}
							}
						}
 ...
 //  		if(isset($_POST['Question']))
 //  		{
 //        $modelQues->ques_title = $_POST['Question'][$id];
 //        $modelQues->ques_explain = $_POST['Explain'][$id];
 //        $key = $id;
 //        if($modelQues->save()){
 //            $answerValidCheckArray = $_POST['Choice'][$key];
 //            $choiceTitle = $_POST['ChoiceTitle'][$key];
 //            Coursechoice::model()->deleteAll('ques_id='.$key);
 //            if (count($choiceTitle) > 0) {
 //                foreach ($choiceTitle as $choiceKey => $choice) {
 //                  $choiceModel = new Coursechoice();
 //                  $choiceModel->ques_id = $modelQues->ques_id;
 //                  $choiceModel->choice_detail = $choice;
 //                  $choiceModel->choice_type = $questionType;
 //                  if (in_array($choiceKey, $answerValidCheckArray)) {
 //                    $choiceModel->choice_answer = 1;
 ...
 //  		));
	...
	public function actionIndex($id)
	{
		...
		if(isset($id))
		{
			...
			$model = new Coursequestion('search');
			$model->unsetAttributes();

			if(isset($_GET['Coursequestion'])){
				$model->attributes = $_GET['Coursequestion'];
			}
	...
	{
		$model=Coursequestion::model()->questioncheck()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	{
		$model=Coursegrouptesting::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
