		...
		if( $assignSelectOptions!==array() )
		{
		    ...
		    if( isset($_POST['AssignmentForm'])===true )
			{
				...
				if( $formModel->validate()===true )
				{
					...
					// add/remove status admin for user
				$userid = Authassignment::model()->findByAttributes(array('userid'=>$model->getId()));
				$getuser = user::model()->findByPk($model->getId());
					$status = $userid->itemname;
					if ($status == 'Admin') {
						$getuser->superuser = '1';
						$getuser->save(false);
					} elseif ($status != 'Admin') {
	...
	public function actionRevoke()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			if( isset($_POST['ajax'])===false )
				// add/remove status admin for user
				$get_id = $_GET['id'];
				$userid = Authassignment::model()->findByAttributes(array('userid'=>$get_id));
				$getuser = user::model()->findByPk($get_id);
					$status = $userid->itemname;
					if ($status == 'Admin') {
						$getuser->superuser = '1';
						$getuser->save(false);
