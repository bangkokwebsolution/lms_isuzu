...
class AuthItemController extends RController
{
	...
	*/
	...
	public function actionAjaxSetCanLogin()
	{
		if (!Yii::app()->request->isAjaxRequest) {
			throw new CHttpException('403', 'Forbidden access.');
		}
		if (empty($_POST['status'])) {
			throw new CHttpException('404', 'Missing "status" POST parameter.');
		}
		$name = $_POST['name'];
		$status = $_POST['status'];
		$authitem = Authitem::model()->findByAttributes(array('name'=>$name));
		$authitem->canLoginBackEnd = $status=='check'?'y':'n';
