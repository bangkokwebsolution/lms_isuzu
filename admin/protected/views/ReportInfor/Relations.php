...
<?php
$titleName = 'ช่องทางการประชาสัมพันธ์';
...
<div class="innerLR">
    ...
    $this->widget('AdvanceSearchForm', array(
        ...
        'attributes'=>array(
            ...
            array('name'=>'contactfrom','type'=>'list','query'=>$contactfrom),
                ...
                <div class="widget-body div-table">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <th class="center" style="width: 150px;">ช่องทางการประชาสัมพันธ์</th>
                   ...
                   if ($DataProvinceInfor != "") {
                        $sqlProvince = "SELECT *,count(firstname) as user FROM  tbl_profiles
                        INNER JOIN tbl_users ON id = user_id
                        WHERE status='1' AND generation='".$DataProvinceInfor['generation']."' ";
                        ...
                        $sqlProvince .= " AND tbl_users.create_at between '".$startDate."' and '".$endDate."' ";
                           if($model->contactfrom !='') {
                                $sqlProvince .= " AND tbl_profiles.contactfrom = '".$model->contactfrom."' ";
                            }

                        $sqlProvince .= ' GROUP BY contactfrom';
                        $provinceData = Yii::app()->db->createCommand($sqlProvince)->queryAll();
                        foreach ($contactfrom as $key => $from) {
                            $count[$from] = 0;
                            foreach ($provinceData as $key => $reprotPro) {
                                $con = explode(',', $reprotPro['contactfrom']);
                                if(in_array($from, $con)){
                                    $count[$from]++ ;
                                    $data .= "['" . $reprotPro['contactfrom']. "',   " . $reprotPro['user'] . "],"; 
                                }
                            }
                        ...
                        <tr>
                            ...
                            <td class="center"><?php echo $count[$from]; ?></td>
                            <td class="center"><?php echo $reprotPro['create_at']; ?></td>
                        </tr>
                        <?php $i++; $sum += $count[$from];
                        } 
                    }
                         ?>
                          <tr>
...
Yii::app()->clientScript->registerScript('chart', "
    ...
    // Load the Visualization API and the piechart package.
      ...
      function drawChart() {
        ...
        data.addColumn('number', '%จำนวนผู้เรียนจากช่องทางการประชาสัมพันธ์');
        data.addRows([
          ".$data."
        ]);
        ...
        // Set chart options
        var options = {
        'title':'%จำนวนผู้เรียนจากช่องทางการประชาสัมพันธ์',
        'width':'%100',
        is3D: true,
        //'height':300
        };
