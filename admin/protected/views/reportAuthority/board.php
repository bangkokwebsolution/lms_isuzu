    ...
    <div class="col-md-12">
       ...
       <table class="table table-bordered" id="user_board">
        ...
        <tbody>
          ...
          if(!empty($user_board)){
            $no = 1;
            foreach ($user_board as $key => $value) {
              ?>
              <tr>
      ...
      <br>
      <button type="submit" class="btn btn-success">+ เพิ่ม ผู้บริหาร</button>
      </form>
...
<script type="text/javascript">
  ...
  function Deleteuser(user) {
    if(user != ""){
      $.ajax({
        type: 'POST',
        url: '<?= $this->createUrl('ReportAuthority/Board') ?>',
        data: ({
          user_id:user,
        }),
