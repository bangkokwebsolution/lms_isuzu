            ...
            InitialSortTable();
                    ...
                    'columns'=>array(
                        ...
                        array(
                            'header'=>'คะแนนการสอบ',
                            'value'=> function($data){
                                return $data->score_number.'/'.$data->score_total.' คะแนน';
                            },
                            'htmlOptions'=>array( 'style'=>'width:150px;' ),
                        ),
                        array(
...
<script>
    $(document).ready(function(){
        $('body').on('click','.learn-reset',function(){
            ...
            $.ajax({
                type: 'POST',
                url: "<?=Yii::app()->createUrl('LearnReset/resetScoreLesson');?>",
                data:{ stdID:stdID,scoreID:scoreID },
                success: function(data) {
                    alert("Reset การเรียน เรียบร้อยแล้ว");
                    window.location.href="<?= $this->createUrl('LearnReset/showResetExam?userID='); ?>"+stdID;
                }
            })
        });
    });
