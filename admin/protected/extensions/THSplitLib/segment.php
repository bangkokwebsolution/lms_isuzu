...
class Segment {
    ...
    public function get_segment_array($input_string) {
        ...
        foreach ($this->_input_string_exploded as $key => $input_string_exploded_row) {
            ...
            foreach ($current_array_result as $each_result) {
                // if (trim($each_result) != '')
                    $this->_segmented_result[] = $each_result;
            }
            if($key != (count($this->_input_string_exploded)-1)){
