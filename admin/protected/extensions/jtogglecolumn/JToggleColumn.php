...
EOD;
    ...
    protected function initDefaultButtons() {
        ...
        if ($this->action === null)
            ...
            $this->toggle_button = array(
                'url' => 'Yii::app()->controller->createUrl("' . $this->action . '",array("id"=>$data->primaryKey,"attribute"=>"' . $this->name . '","check"=>"' . $this->checkPoint . '"))',
                'options' => array('class' => $this->name . '_toggle'),
            );
...
EOD;
    ...
    protected function renderHeaderCellContent() {
        if ($this->grid->enableSorting && $this->sortable && $this->name !== null)
            echo $this->grid->dataProvider->getSort()->link($this->name, $this->header,array('class'=>'sort-link','style'=>'color:white;'));
        ...
        if ($this->filter !== null) {
            if (is_string($this->filter))
                ...
                if (is_array($this->filter))
                    echo CHtml::activeDropDownList($this->grid->filter, $this->name, $this->filter, array('id' => false, 'prompt' => 'ทั้งหมด'));
