...
class CourseApiMobileController extends Controller
{
    ...
    public function actionGetLesson($course_id = null, $lang_id = null)
    {
        if (empty($course_id) || empty($lang_id)) {
            echo "false";exit;
        }
        $model = Lesson::model()->findAll(['condition' => 'lang_id = 1 AND active = "y" AND parent_id = 0  AND course_id = ' . $course_id]);

        $arr_course = [];
        if ($lang_id == 1) {
            foreach ($model as $key => $val) {
                $arr_course[] = $this->ArrLesson($val);
            }
        } elseif ($lang_id == 2) {
            foreach ($model as $key => $val) {
                $model_ck = Lesson::model()->find(['condition' => 'lang_id = ' . $lang_id . ' AND active = "y" AND parent_id = ' . $val->id . ' AND course_id = ' . $course_id]);
                if (!empty($model_ck)) {
                    $arr_course[] = $this->ArrLesson($model_ck);
                } else {
                    $arr_course[] = $this->ArrLesson($val);
                }
            }
    ...
    private function ArrLesson($model)
    {
        $arr_course = [
            ...
            'cate_percent' => $model->cate_percent,
            ...
            'active' => $model->active,
            'view_all' => $model->view_all,
            'status' => $model->status,
            'lesson_no' => $model->lesson_no,
            'type' => $model->type,
