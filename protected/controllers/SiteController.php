...
class SiteController extends Controller
{
	...
	// 	var_dump($list);
	// }
	public	function actionDashboardApi()
	{
			...
			foreach ($start_course as $key => $value) {
				if (isset($value->course)) {
					...
					if (!empty($value->course->course_picture) && file_exists(YiiBase::getPathOfAlias('webroot') . '/uploads/courseonline/' . $course_id . '/original/' . $value->course->course_picture)) {
						$image = Yii::app()->createUrl("uploads/courseonline") . '/' . $course_id . '/original/' . $value->course->course_picture;
					} else {
						$image = Yii::app()->theme->baseUrl . "/images/course-image.png";
					}
					...
					$data_total[] = ['course_id' => $course_id, 'course_title' => $value->course->course_title, 'picture' => $image, 'status' => $status_text, 'percentage' => $percentage, 'start_date' => $date_start, 'end_date' => $date_stop];
				}
			}
			$data = ['status_api' => 'success', 'data' => !empty($data_total) ? $data_total : ['null' => 'null']];
		} else {
