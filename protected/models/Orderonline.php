...
class Orderonline extends CActiveRecord
{
	...
	public function relations()
	{
		return array(
			...
			'OrderDetailonlines'=>array(self::HAS_MANY, 'OrderDetailonline', 'order_id'),
    ...
    public function getCheckPaysBuy()
    {
    	...
    	{
			$model = OrderDetailonline::model()->findAll(array(
    		...
    		{
    			$i++;
    			$productVariable.= '('.$i.') '. $value->courseOnline->course_title.' ';
    		}

			//$productVariable 	= 'Detail';
			$postURL = "http://www.cpdland.com/index.php/orderonline/payment";
	...
	public function getOrderId($text = 'A')
	{
	...
	public function getCheckConfirm()
	{
		...
		if($this->con_user == 0)
		{
			$link = CHtml::link(CHtml::image($imageUrl,'Accept'),array('update','id'=>$this->order_id),array(
			   	'class' => 'imageIcon',
			));
		}
		else
	...
	public function getCheckConfirmIndex()
	{
		$imageUrl = Yii::app()->request->baseUrl .'/images/icons/coins.png';
		...
		else
		{
			...
			{	
				$link = '
					<font color="#009933" style="font-weight:bold;"> 
					การแจ้งโอนเรียบร้อย
					</font>
				';
			}
	...
	public function getOrderDateAdd()
	{
		if(isset($this->order_date_add))
			$date = 'วันที่ '.ClassFunction::datethai($this->update_date);
		else
			$date = '<font style="text-align:center;" color="red">ยังไม่ได้ยืนยัน</font>';
	...
	public function getPrintReceipt()
	{
		...
		if($this->con_admin == '1')
			...
			$print = '<font style="text-align:center;" color="red">รออนุมัติ</font>';
	...
	public function defaultScope()
	{
	    return array(
	    	'alias'=>'orderonlines',
	    	'order' => 'orderonlines.order_id desc',
	    	'condition' => ' orderonlines.active = "y" AND orderonlines.user_id = "'.Yii::app()->user->id.'" ',
	    );
	}
