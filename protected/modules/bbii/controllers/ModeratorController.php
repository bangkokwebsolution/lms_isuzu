...
class ModeratorController extends BbiiController {
	...
	public function accessRules()
	{
		return array(
			array('allow',
				'actions'=>array('admin','approval','approve','banIp','changeTopic','delete','ipAdmin','ipDelete','view','refreshTopics','report','topic','sendmail','keywordAdmin','keywordDelete','keywordAdd'),
				'users'=>array('@'),
	...
	public function actionKeywordAdmin()
	{
		$model=new BbiiIKeyword('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['BbiiIKeyword']))
			$model->attributes=$_GET['BbiiIKeyword'];

		$this->render('keywordadmin',array(
			'model'=>$model,
		));
	}

	public function actionKeywordAdd()
	{
		$model=new BbiiIKeyword();
	...
	public function actionKeywordDelete($id) {
		BbiiIKeyword::model()->findByPk($id)->delete();

		// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
		if(!isset($_GET['ajax']))
			$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('keywordadmin'));
	}
